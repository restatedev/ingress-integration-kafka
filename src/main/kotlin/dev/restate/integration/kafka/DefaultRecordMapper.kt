...
/**
 * Default [RecordMapper], with parity to the built-in Kafka consumer: the Kafka key becomes the
 * VO/Workflow key, the value becomes the payload, and `kafka.*` metadata is attached as headers.
 * W3C trace context is propagated from the record headers when present. No content-type is set.
 */
object DefaultRecordMapper : RecordMapper {
  ...
  override fun toRecord(
      ...
      key: String?,
      value: ByteArray?,
      headers: List<Pair<String, ByteArray>>,
  ): Record {
    val builder =
        Record.newBuilder()
            .setOffset(offset)
